...
렌더러는 에이전트로 보내는 메시지의 **metadata**에 `a2uiClientCapabilities` 객체를 포함합니다(예: A2A `Message`의 `metadata` 필드).
...
multi-agent 아키텍처에서 라우팅을 처리하려면 orchestrator가 각 `surfaceId`와 소유 sub-agent의 매핑을 유지해야 합니다. 공식 Python SDK는 이 매핑을 안전하게 관리할 수 있도록 [`A2uiSubagentMap`](../../../agent_sdks/python/a2ui_agent/src/a2ui/adk/orchestration/a2ui_subagent_map.py) 유틸리티를 제공합니다.

전체 동작 예시는 [orchestrator agent executor](../../../samples/community/agent/adk/orchestrator/orchestrator_agent_executor.py) 샘플을 참고하세요.

#### 1. 서버 이벤트로부터 Ownership 매핑

sub-agent가 A2UI `createSurface` 또는 `deleteSurface`를 내보내면, orchestrator는 해당 메시지를 가로채 `update_from_server_event`를 사용해 session state에 ownership을 기록합니다.
...
클라이언트 렌더러가 A2UI `action` 또는 `error` 메시지를 되돌려 보내면, orchestrator는 `get_subagent_name_for_client_event`를 사용해 session state에서 `surfaceId`를 조회하고 요청을 올바른 sub-agent로 라우팅합니다.
...
이 패턴은 각 기능 영역별로 양방향 통신 루프가 온전하고 stateful하게 유지되도록 보장합니다.

### Metadata Stripping으로 데이터 누출 방지

multi-agent 환경에서 `a2uiClientDataModel` 객체에는 서로 다른 sub-agent가 소유한 여러 surface의 상태가 포함될 수 있습니다. 민감한 데이터 누출을 방지하려면 orchestrator가 대상 sub-agent가 소유한 surface만 포함되도록 데이터 모델 metadata를 **strip**해야 합니다.

outbound interceptor에서 `strip_unowned_surfaces_from_data_model` 메서드를 사용해 데이터 모델 딕셔너리를 제자리에서(in place) 변형할 수 있습니다.
...
metadata를 strip함으로써 orchestrator는 각 sub-agent가 볼 권한이 있는 데이터 모델 부분만 받도록 보장합니다.

WARNING: **보안 위험 - 상태 스크래핑**: orchestrator가 `a2uiClientDataModel`을 strip하지 않으면, 악성 또는 침해된 sub-agent가 다른 활성 surface의 상태를 읽을 수 있습니다. 예를 들어 orchestrator가 전체 multi-surface 데이터 모델을 누출하면 날씨 sub-agent가 은행 surface의 민감한 데이터를 scrape할 수 있습니다. multi-agent 시스템에서 stripping은 필수 보안 요구사항입니다.
