    ...
    ```json
    {
      ...
      "updateComponents": {
        ...
        "components": [
          ...
          {"id": "greeting", "component": "Text", "text": "안녕하세요"},
          ...
          {"id": "cancel-text", "component": "Text", "text": "취소"},
          {"id": "ok-btn", "component": "Button", "child": "ok-text", "action": {"event": {"name": "ok"}}},
          {"id": "ok-text", "component": "Text", "text": "확인"}
        ]
      }
    }
    ```

    v0.9 이상에서는 더 평면적인 컴포넌트 형식을 사용합니다. 중첩된 `{"Text": {...}}` 대신 `"component": "Text"`를 사용하고, 자식은 `{"explicitList": [...]}` 대신 단순 배열로 표현합니다.
...
=== "v0.9 and later"
    ...
    ```json
    {"id": "welcome", "component": "Text", "text": "안녕하세요", "variant": "h1"}
    ```
...
=== "v0.9 and later"
    ...
    1. LLM이 `createSurface`를 통해 서피스를 만듭니다(카탈로그 지정, v1.0에서는 초기 데이터 모델과 컴포넌트도 함께 포함할 수 있습니다).
    2. LLM이 `updateComponents`를 통해 컴포넌트 정의를 생성합니다.
    3. LLM이 `updateDataModel`을 통해 데이터를 채웁니다.
    4. 클라이언트가 모든 컴포넌트를 네이티브 위젯으로 렌더링합니다.
...
구현 세부 사항은 [커스텀 컴포넌트 가이드](../guides/authoring-components.md)를 참조하세요.
