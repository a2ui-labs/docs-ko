...
이 페이지는 모든 표준 A2UI 컴포넌트를 예시 및 사용 패턴과 함께 소개합니다. 전체 기술 명세는 [표준 카탈로그 정의](https://github.com/a2ui-project/a2ui/blob/main/specification/0.8/json/standard_catalog_definition.json)를 참조하세요.
...
> [!NOTE]
> **패키지 관리자 사용:** A2UI 저장소 안에 포함된 샘플 애플리케이션을 실행하려면 Corepack 워크스페이스 설정에 따라 Yarn(`yarn start gallery`)이 필요합니다. 이 저장소 밖에서 일반적인 용도로 사용하거나 독립된 프로젝트를 구성할 때는 원하는 패키지 관리자(예: npm, pnpm)를 사용하면 됩니다.
